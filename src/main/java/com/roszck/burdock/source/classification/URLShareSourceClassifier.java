...
class URLShareSourceClassifier implements ShareSourceClassifier {
    ...
    public SourceLabeledShare classify(Share share) {
        ...
        if(!HTTP_PROTOCOL.equals(source.getProtocol()) && !HTTPS_PROTOCOL.equals(source.getProtocol()))
            return new ErrorShare(share, String.format("Detected protocol not allowed. Detected: %s", source.getProtocol()));
        if(TWITTER_HOST.equals(source.getHost()))
            return new TwitterShare(share.getUserId(), source);
        if(YOUTUBE_HOST.equals(source.getHost()))
            return new YoutubeShare(share.getUserId(), source);
        return new WebPageShare(share.getUserId(), source);
    }
